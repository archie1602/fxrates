...
func New(httpClient *http.Client, baseURL string, retry RetryPolicy) (*Client, error) {
	...
	if retry.Delay < 0 {
		return nil, errors.New("create frankfurter client: retry delay must not be negative")
	}
...
func (c *Client) FetchRate(ctx context.Context, pair domain.Pair) (service.RateSnapshot, error) {
	...
	for attempt := 1; attempt <= c.retry.MaxAttempts; attempt++ {
		snapshot, retry, err := c.fetchRateOnce(ctx, endpoint, validatedPair, base, quote)
		if err == nil {
			return snapshot, nil
		}
		...
		if err := waitForRetry(ctx, c.retry.Delay); err != nil {
...
func (c *Client) fetchRateOnce(
	...
	base string,
	quote string,
) (service.RateSnapshot, bool, error) {
	request, err := http.NewRequestWithContext(ctx, http.MethodGet, endpoint.String(), nil)
	if err != nil {
		return service.RateSnapshot{}, false, fmt.Errorf("fetch Frankfurter rate: create request: %w", err)
	}
	request.Header.Set("Accept", "application/json")

	response, err := c.httpClient.Do(request)
	if err != nil {
		return service.RateSnapshot{}, ctx.Err() == nil, fmt.Errorf("fetch Frankfurter rate: send request: %w", err)
	}
	defer func() {
	...
	if response.StatusCode < http.StatusOK || response.StatusCode >= http.StatusMultipleChoices {
		...
		return service.RateSnapshot{}, retry, decodeUpstreamError(response)
	}

	body, err := readLimitedBody(response.Body)
	if err != nil {
		return service.RateSnapshot{}, false, fmt.Errorf("fetch Frankfurter rate: read response: %w", err)
	}
	...
	if err := decoder.Decode(&payload); err != nil {
		return service.RateSnapshot{}, false, fmt.Errorf("fetch Frankfurter rate: decode response: %w", err)
	}
	if err := decoder.Decode(&struct{}{}); !errors.Is(err, io.EOF) {
		return service.RateSnapshot{}, false, errors.New("fetch Frankfurter rate: response must contain one JSON object")
	}

	if payload.Base != base || payload.Quote != quote {
		return service.RateSnapshot{}, false, fmt.Errorf(
	...
	if err != nil {
		return service.RateSnapshot{}, false, fmt.Errorf("fetch Frankfurter rate: response contains an invalid rate: %w", err)
	}

	rateDate, err := time.Parse(time.DateOnly, payload.Date)
	if err != nil {
		return service.RateSnapshot{}, false, fmt.Errorf("fetch Frankfurter rate: parse rate date: %w", err)
	}
